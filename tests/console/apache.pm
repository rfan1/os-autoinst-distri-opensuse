...
sub run {
    ...
    if (is_sle('12-SP2+')) {
        if (check_var('SYSTEM_ROLE', 'kvm') || check_var('SYSTEM_ROLE', 'xen')) {
            ...
            # use '/usr/lib64/apache2-prefork/mod_authn_file.so' for sle16.1
            assert_script_run q{sed -i 's#/usr/lib64/apache2#/usr/lib64/apache2-prefork#g' /tmp/prefork/httpd.conf};
