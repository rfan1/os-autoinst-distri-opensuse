...
# Taselect_serial_terminalgs: poo#19606
...
sub run {
    ...
    my @orphans = split('\n',
        script_output
        q[zypper --quiet packages --orphaned | tee -a /tmp/orphaned.log | awk -F'|' 'intable && NR>2 {print $3} /^-/ { intable=1 }' | grep -v '\(release-DVD\|release-dvd\|openSUSE-release\|skelcd\)'],
